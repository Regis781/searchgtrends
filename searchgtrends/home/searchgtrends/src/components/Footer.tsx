      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 24px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 40, marginBottom: 48 }}>
          <div style={{ gridColumn: 'span 1' }}>
            ...
            <p style={{ fontSize: 12, color: 'rgba(240,242,248,0.35)', lineHeight: 1.7, maxWidth: 220 }}>
              Les mots les plus recherchés sur Google dans 10 pays. Mis à jour chaque matin à 8h.
            </p>
          </div>
          {[
            ...
            { title: 'À propos', links: [['/a-propos', 'Notre mission'], ['/methodologie', 'Méthodologie'], ['/a-propos', 'Roadmap'], ['/a-propos', 'Stack technique']] },
          ].map(({ title, links }) => (
            <div key={title}>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {links.map(([to, label]) => (
                  <li key={label}>
                    <Link to={to} style={{ fontSize: 13, color: 'rgba(240,242,248,0.45)', textDecoration: 'none', transition: 'color 0.15s' }}
                      onMouseEnter={e => (e.currentTarget.style.color = '#a5b4fc')}
                      onMouseLeave={e => (e.currentTarget.style.color = 'rgba(240,242,248,0.45)')}>
                      {label}
                    </Link>
                  </li>
                ))}
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            <Link to="/methodologie" style={{ fontSize: 12, color: 'rgba(240,242,248,0.2)', textDecoration: 'none' }}>Méthodologie</Link>
            <Link to="/a-propos" style={{ fontSize: 12, color: 'rgba(240,242,248,0.2)', textDecoration: 'none' }}>À propos</Link>
